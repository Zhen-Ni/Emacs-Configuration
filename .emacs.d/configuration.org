...
#+begin_src emacs-lisp
  (defvar-local hs-toggle-state 'hide-all "Record python block hide-show status.")
...
Use special ellipsis than vanilla "..." for hided text, and show line counts.
...
#+begin_src emacs-lisp
  (defun python-hs-display-code-line-counts (ov)
    "Display custom indicator for hidden code indicated by OV."
    (when (eq 'code (overlay-get ov 'hs))
      (let* ((ellipsis-str (format " %s " (if (char-displayable-p ?⤸) "..⤸" "...")))
