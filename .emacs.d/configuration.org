...
#+begin_src emacs-lisp
  ...
  (defun set-theme ()
    "Interactively set Emacs theme pairs based on contrast level: 'high' or 'medium'."
    (interactive)
    (let ((choice (completing-read "Choose contrast level (high/medium/low): " '("high" "medium" "low") nil t)))
      (cond
       ((string= choice "high")
        (setq theme-light 'modus-operandi-tinted)
        (setq theme-dark 'modus-vivendi-tinted)
        (message "Loaded high contrast theme: modus"))
       ((string= choice "medium")
        (setq theme-light 'tango)
        (setq theme-dark 'tango-dark)
        (message "Loaded medium contrast theme: tango"))
       ((string= choice "low")
