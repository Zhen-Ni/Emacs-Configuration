...
**** COMMENT Projectile Mode Line

*Use the built-in project mode line. Projectile mode line is suppressed
in mode line settings*

Use Abbreviation for `Projectile`.

#+begin_src emacs-lisp
  (setq projectile-mode-line-prefix
        (if (and t (all-the-icons-available))          ;; TO BE FINISHED!!!!
  ...
  (defun projectile-simplified-mode-line ()
  ...
  ;; Hide project type by self-defined mode line funciton.
  (setq projectile-mode-line-function #'projectile-simplified-mode-line)
#+end_src
...
*** COMMENT Hide Unnecessarily Displayed Modes

To hide the minor mode displays in the mode line. Some modes cannot be
simply hided using ~delight~ package, such as ~projectile~ due to its
dynamic parameters, which will be handled seperately in the mode's
configuration.
...
**** Calendar
The original keybinding for ~org-agenda~ is ~C-c a~. But it is binded to
[[Aidermacs]]. Bind it to ~C-c c~ instead.

#+begin_src emacs-lisp
  (add-hook 'org-mode-hook
            (lambda ()
              (define-key org-mode-map (kbd "C-c c") #'org-agenda)
  	    ))
#+end_src
...
*This mode is disabled because of too many inconsistent behaviors with
the built-in one.*
...
#+begin_src emacs-lisp
  (if (eq 'gnu/linux system-type)
      ...
      (setq flycheck-python-mypy-cache-dir
  	  "/home/collie/.emacs_backups/.mypy_cache/")
    ;; For Windows system.
    (setq flycheck-python-mypy-cache-dir
  	"C:/cache/.mypy_cache/")
    )
