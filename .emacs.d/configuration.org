...
#+begin_src emacs-lisp
  (defun python-set-project-root ()
    "Set Python working directory to project root."
    (let ((project-root (projectile-project-root)))
      (when project-root
        (setq-local default-directory project-root))))

  (add-hook 'python-mode-hook #'python-set-project-root)
#+end_src
...
Pylsp becomes very slow when dealing with large libraries such as
Transformers, which can cause Emacs to freeze. Use Pyright instead if
~pyrightconfig.json~ can be found in the path. See setup instructions on
[[https://emacs-lsp.github.io/lsp-pyright/][this site]].
