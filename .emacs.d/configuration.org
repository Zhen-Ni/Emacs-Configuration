...
#+begin_src emacs-lisp
  ...
  (defun python-shell-buffer-show-string (s)
    "TBD"
    ())
  ...
  (defun python-run-buffer-or-selected-region-helper (&optional run_main)
    ...
    (if (use-region-p)
        (progn
  	(python-shell-buffer-show-string (buffer-substring (region-beginning) (region-end)))
  	(python-shell-send-region (region-beginning) (region-end) run_main)
  	)
      (python-shell-buffer-show-string (format "<Eval buffer \"<%s>\">" (buffer-name)))
